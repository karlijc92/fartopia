...
export default function Habitat() {
  ...
  const unlockHabitat = useMutation({
    ...
    mutationFn: async () => {
      ...
      if (currentCoins < habitat.cost) {
        throw new Error("Not enough coins");
      }

      const newUnlocked = [
        ...(progress.unlocked_habitats || []),
        habitatId
      ];

      const newCoins = currentCoins - habitat.cost;

      const result = await base44.entities.GameProgress.update(progress.id, {

        unlocked_habitats: newUnlocked,
  ...
  const creaturesInHabitat =
    creatures?.filter(
      (c) => c.in_habitat === habitatId && c.unlocked
    ) || [];
        ...
        {!isUnlocked ? (
          ...
          >
            ...
            <p className="text-2xl mb-8">
              Unlock this habitat to place creatures here!
            </p>
