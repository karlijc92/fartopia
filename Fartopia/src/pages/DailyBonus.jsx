...
export default function DailyBonus() {
  ...
  useEffect(() => {
    if (progress?.last_daily_claim) {
      ...
      const isSameDay = lastClaim.toDateString() === today.toDateString();
  ...
  const unlockedCreatures = creatures?.filter(c => c.unlocked).length || 0;
  const unlockedHabitats = progress?.unlocked_habitats?.length || 0;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-200 via-orange-200 to-pink-200 p-6">
      <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-6">
              ...
              <div className="text-white">
                <h2 className="text-4xl font-black mb-2">Daily Bonus!</h2>
                <p className="text-xl mb-2">Come back every day for free coins!</p>
                {progress?.daily_streak > 0 && (
                  <div className="flex items-center gap-2">
                    <Zap className="h-5 w-5" />
                    <p className="text-lg font-bold">{progress.daily_streak} Day Streak! 🔥</p>
                  </div>
                )}
              </div>
            </div>
            <div className="text-center">
              {claimedToday ? (
                <div className="bg-white/30 text-white font-bold text-xl px-12 py-6 rounded-full">
                  ✓ Claimed Today!
                  <div className="text-sm mt-1">Come back tomorrow!</div>
                </div>
              ) : (
                <Button
                  onClick={handleClaim}
                ...
                >
                  ...
                  Claim {100 + Math.min((progress?.daily_streak || 0) * 10, 100)} Coins!
                </Button>
              )}
            </div>
          </div>
        ...
        {/* Achievements */}
        <div className="bg-white/50 backdrop-blur-sm rounded-3xl p-6 shadow-xl mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {ACHIEVEMENTS.map((achievement, index) => {
              ...
              if (achievement.id === 'first_creature') isUnlocked = unlockedCreatures >= 1;
              if (achievement.id === 'five_creatures') isUnlocked = unlockedCreatures >= 5;
              if (achievement.id === 'first_habitat') isUnlocked = unlockedHabitats >= 1;
              if (achievement.id === 'three_habitats') isUnlocked = unlockedHabitats >= 3;
              if (achievement.id === 'high_score') isUnlocked = (progress?.high_score || 0) >= 50;
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    {isUnlocked && <Star className="h-6 w-6 text-yellow-600" />}
                  </div>
                  ...
                  <div className={`font-bold ${isUnlocked ? 'text-white' : 'text-gray-600'}`}>
                    {isUnlocked ? '✓ Unlocked!' : `🪙 ${achievement.reward} coins`}
                  </div>
                </motion.div>
              );
            })}
        ...
        <motion.div
          ...
          className="bg-gradient-to-br from-purple-400 to-pink-500 rounded-3xl p-8 shadow-xl text-white text-center"
        >
          <Crown className="h-16 w-16 mx-auto mb-4" />
          <h2 className="text-3xl font-bold mb-2">Weekend Bonus Event! 🎉</h2>
          <p className="text-xl mb-4">Play games on weekends for DOUBLE coins!</p>
          <p className="text-lg">Check back Saturday & Sunday for special rewards!</p>
        </motion.div>
      </div>
    </div>
  );
